<template>
    <div class="overflow-content">
        ...
        <div class="srow content spaced" @keyup.enter="loginPlain(user, password)">
            <h2 class="alignCenter"><span>{{ $t('login') }}</span></h2>
            <div class="eleven columns offset-by-one">
                        ...
                        <div class="eleven columns">
                            <div class="srow saved-user" v-for="username in allUsersList" style="margin-bottom: 0">
                                <div :class="username === activeUser ? 'loggedIn' : ''">
                                    ...
                                    <button class="three columns" @click="loginStored(username)" v-show="hasValidMajorModelVersion(username)">
                                        <span>{{ $t('open') }}</span>
                                    ...
                                    <button class="two columns" @click="removeStoredUser(username)" v-show="hasValidMajorModelVersion(username)">
                                    ...
                                    <button class="two columns" @click="deleteAccount(username)" v-show="hasValidMajorModelVersion(username) && savedOnlineUsers.includes(username)">
                                        <span>{{ $t('deleteAccount') }}</span> <i class="fas fa-user-slash"></i>
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="srow">
                        <form autocomplete="on" class="eleven columns saved-user" onsubmit="event.preventDefault()">
                            ...
                            <div class="srow">
                                ...
                                <input type="text" name="username" autocapitalize="none" v-model="user" id="inputUser" class="four columns" autocomplete="username"/>
                            ...
                            <div class="srow" v-show="!savedUsers.includes(user)">
                                ...
                                <button type="button" @click="loginPlain(user, password)" :disabled="!user || !password" class="five columns offset-by-one">
                                    <span>{{ $t('login') }}</span>
                                    <span> <!-- todo affichage des raisons pour v-show="loginSuccess != true" -->
                ...
                <div v-show="allUsersList.length == 0">
                    <form autocomplete="on" onsubmit="event.preventDefault()">
                        <div class="srow">
                            <label for="inputUser2" class="four columns"><span class="desktop-right">{{ $t('username') }}</span></label>
                            <input type="text" name="username" autocapitalize="none" v-model="user" id="inputUser2" class="four columns" autocomplete="username"/>
                        </div>
                        <div class="srow">
                            <label for="inputPassword2" class="four columns"><span class="desktop-right">{{ $t('password') }}</span></label>
                    ...
                    <div class="srow alignCenter">
                        <button @click="loginPlain(user, password)" :disabled="!user || !password" class="four columns floatNone">{{ $t('login') }}</button>
                    </div>
                    <div class="srow">
                        <div class="eleven columns offset-by-four">
                            ...
                            <div v-show="loginSuccess === false">
                                <span v-if="loginErrorCode === loginService.ERROR_CODE_INACTIVE_ACCOUNT || loginErrorCode === loginService.ERROR_CODE_LOCKED" class="account-error inactive-account">
                                    <i style="color:var(--col-advert)" class="fas fa-user-lock"></i>{{loginErrorCode | translate}}
                                </span>
                                <span v-else  class="account-error">
                                    <i style="color: red" class="fas fa-times"/>{{loginErrorCode | translate}}
                                </span>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteAccount(user) {
                if (!confirm(i18nService.t('CONFIRM_DELETE_ACCOUNT', user))) {
                    return;
                }
                console.info("*** deleteAccount ***");
                databaseService.deleteUserAccount(user).then(() => {
                    console.info("*** deleteUserAccount ***");
                    localStorageService.removeLocalUser(user);
                    if (loginService.getLoggedInUsername() === user) {
                        console.info("*** logout ***");
                        loginService.logout();
                    ...
                    alert(i18nService.t('ERROR_DELETING_ACCOUNT'));
