<template>
    <div class="overflow-content">
        ...
        <div class="srow content spaced">
            <h2 class="alignCenter"><span>{{ $t('registerOnlineUser') }}</span></h2>
            <form autocomplete="off" onsubmit="event.preventDefault()" style="margin-bottom: 0.5em">
                <div class="srow">
                    ...
                    <input type="text" name="inputUser" autocapitalize="none" v-model="user" id="inputUser" class="six columns" @input="validateUsername" v-focus="" maxlength="16"/>
                ...
                <div class="srow">
                    <label for="email" class="two columns inputlabel"><span class="desktop-right">{{ $t('yourEmail') }}</span></label>
                    <input type="text" v-model="email" name="email" autocapitalize="none" required id="email" class="six columns" @input="validateEmail" />
                    <div class="three columns" v-show="!emailValid && email.length > 0"">
                        <i style="color: red;" class="fas fa-times"/> <span>{{ $t('pleaseEnterAValidEmailAddress') }}</span>
                    </div>
                    <div class="three columns" v-show="emailValid">
                        <i style="color: green;" class="fas fa-check"/> <span>{{ $t('validEmailAddress') }}</span>
                    </div>
            ...
            <div class="srow more-space">
                <div class="six columns offset-by-two" v-show="!!password && password2 !== null && password === password2">
                    ...
                    <ul style="list-style-type: none; margin-bottom: 0">
                        ...
                        <li>{{ $t('accessToApplicationRequiresSubscription') }}</li>
            ...
            <div class="srow more-space">
                <div class="six columns offset-by-two whiteBck borderRadius" v-show="!!password && password2 !== null && password === password2">
                    ...
                    <label for="checkPrivacy">
                        <i18n path="iAcceptPrivacyPolicy" tag="span">
                            ...
                            <template v-slot:link>
                                <a href="app/privacy_fr.html?back=register">{{ $t('privacyPolicy') }}</a>
                            </template>
                        </i18n>
                    </label>
                    <br/>
                    ...
                    <span>
                        <i18n path="theOnlyPersonalDataSavedIs" tag="span">
                            <template v-slot:link>
                                <a href="mailto:contact@jemefaiscomprendre.com">contact@jemefaiscomprendre.com</a>
                            </template>
                        </i18n>
                    </span>
                </div>
            ...
            <div class="srow">
                <button @click="register" :disabled="!user || !password || !password2 || password !== password2 || !usernameValid || !privacyConsent ||!emailValid" class="six columns offset-by-two">{{ $t('register') }}</button>
            </div>
            <div class="srow">
                <div class="six columns offset-by-two">
                    <span>{{ $t('alreadyHaveAnAccount') }}</span>
                    <a href="#login">{{ $t('toLogin') }}</a><br>
    ...
    export default {
        ...
        methods: {
            ...
            register() {
                ...
                loginService.register(this.user, this.password, this.remember, this.email).then(() => {
