...
function getGridElementNormal(gridElem, metadata) {
    ...
    var template = `
<li class="item ${gridElem.actions}" data-w="${gridElem.width}" data-h="${gridElem.height}" data-x="${gridElem.posX}" data-y="${
        gridElem.posY
    ...
    <div class="grid-item-content" tabindex="40" aria-label="${ariaLabel}" id="${gridElem.id}" data-id="${
        ...
        <div class="img-container" style="width: 100%; max-height: ${imgContainerMaxHeight};">
            <img src="${imgData}" draggable="false" style="max-width: 85%; max-height: 90%; object-fit: contain; margin: 1%;" crossorigin="anonymous"/>
        </div>
...
function getHintsElement(gridElem) {
    ...
    let navHint =
        gridElem.actions.filter((a) => a.modelName === GridActionNavigate.getModelName()).length > 0
            ? '<i class="fas fa-plus element-hint"></i>'
            : '';
    return `<span style="position: absolute; right: 5px; color: var(--light-blue)">${hiddenHint + ' ' + navHint}</span>`;
}
