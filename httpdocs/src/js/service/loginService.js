 ...
 *
 */
loginService.register = function (user, plainPassword, saveUser, email) {
    _tryUser = user;
    loginService.stopAutoRetryLogin();
    user = user.trim();
    let password = encryptionService.getUserPasswordHash(plainPassword);
    let sqlRegistrationData = null; // Pour stocker la réponse de l'API
    ...
    .then(data => {
        log.info('SQL registration successful:', data);
        sqlRegistrationData = data;

        // 2. Attendre un peu pour s'assurer que l'entrée SQL est bien créée
        return new Promise(resolve => setTimeout(resolve, 1000));
    })
    .then(() => {
        // 3. Enregistrement CouchDB
        log.info('Proceeding with CouchDB registration');
        return superlogin
            .register({
                username: user,
                email: email,
    ...
    .then(async () => {
        log.info('CouchDB registration successful');
    ...
    .catch((reason) => {
        log.error('Registration/Login failed:', reason);
        
        // Si l'erreur vient de la vérification d'abonnement, on peut réessayer
        if (reason === loginService.ERROR_CODE_INACTIVE_ACCOUNT) {
            log.info('Subscription check failed, retrying login...');
            return new Promise(resolve => setTimeout(resolve, 2000))
                .then(() => loginInternal(user, password, saveUser))
                .then(() => {
                    return databaseService.registerForUser(
                        _loggedInUser,
                        password,
        ...
        .then(async (info) => {
            ...
            try {
                ...
                if (response.status === 401) {
                    ...
                    log.warn('Error details:', errorText);
...
function loginHashedPasswordInternal(user, hashedPassword, saveUser) {
    return loginInternal(user, hashedPassword, saveUser).then(
        ...
        (reason) => {
            ...
            // Si c'est une erreur d'abonnement inactif, la propager directement. Envisager en gérant cette erreur différement de permettre l'utilisation hors ligne.   
            if (reason === loginService.ERROR_CODE_INACTIVE_ACCOUNT) {
                return Promise.reject(reason);
            }
...
function reasonToErrorCode(reason) {
    if (reason === loginService.ERROR_CODE_INACTIVE_ACCOUNT) {
        return loginService.ERROR_CODE_INACTIVE_ACCOUNT;
    }
    if (
        reason &&
        reason.error &&
        ...
        return loginService.ERROR_CODE_LOCKED;
    }
